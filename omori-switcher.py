...
def main():
    ...
    # If the file didn't exist, run these lines
        ...
        location = input("Enter your steam path: ")
        f.write(location)
        ...
        while check != 'Y' and check != 'y' and check != 'n' and check != 'N':
            check = input("Is your game currently modded:")
        if check == 'Y' or check == 'y':
            try:
                os.rename(location + '\\Omori', location + "\\Omori (Modded)")
                print("Succefully renamed folder. Now reinstall Omori through Steam, then the script should auto switch your game version between modded and unmodded. Press any key to exit script")
                no = input("")
                sys.exit()
            except OSError:
            ...
            try:
                ...
                print("Succefully renamed folder. Now reinstall Omori through Steam, mod the game, then the script should auto switch your game between modded and unmodded")
                input()
                sys.exit()
            except OSError:
                print("An error occured. This is usually caused by having the game directory open in file explorer while running the script. Please close the file explorer, then run the script again.")
